...
import { drawRenderLayer } from './WebGlDrawingUtils';
...
const Surface = React.createClass({
  ...
  getDefaultProps: function () {
    console.log(`default window ${window.devicePixelRatio}`);
    return {
  ...
  getContext: function () {
    ...
    ) : invariant(this._isMounted));
    return this.refs.canvas.getContext(this.props.contextType);
  },

  scale: function () {
    if (this.props.contextType === '2d') {
